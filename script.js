...
function renderNews(){const all=filtered(),list=expanded?all:all.slice(0,12);$("section-title").textContent=active==="首页"?"把重要新闻，放在同一条信息流里":`${active} · 最新资讯`;$("result-count").textContent=`${all.length} 条结果`;$("news-grid").innerHTML=list.map(a=>{const id=escapeHtml(a.id),tag=escapeHtml(tagOf(a)),source=escapeHtml(a.source),title=escapeHtml(a.title),summary=escapeHtml(a.summary||'点击阅读全文，查看原始媒体报道。'),url=escapeHtml(safeUrl(a.url)),imageUrl=safeUrl(a.image||a.imageUrl||a.thumbnail||'');const visual=imageUrl==='#'?'':`<div class="visual news-image"><img src="${escapeHtml(imageUrl)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer"><span>${tag}</span></div>`;return `<article class="card">${visual}<div class="card-body"><div class="meta"><span>${source}</span><time>${escapeHtml(timeAgo(a.publishedAt))}</time></div><h3>${title}</h3><p>${summary}</p><div class="card-actions"><a class="read" href="${url}" target="_blank" rel="noopener noreferrer">阅读全文 ↗</a><button class="save ${saved.has(a.id)?'saved':''}" data-save="${id}">${saved.has(a.id)?'已收藏':'收藏'}</button></div></div></article>`;}).join("");$("empty").hidden=all.length>0;$("load-more").hidden=expanded||all.length<=12;$("news-grid").querySelectorAll(".news-image img").forEach(img=>img.addEventListener("error",()=>img.closest(".news-image")?.remove(),{once:true}));$("news-grid").querySelectorAll("[data-save]").forEach(b=>b.addEventListener("click",()=>{saved.has(b.dataset.save)?saved.delete(b.dataset.save):saved.add(b.dataset.save);renderNews();}));}
async function loadNews(){
 try{const response=await fetch(`data/news.json?v=${Date.now()}`,{cache:"no-store"});if(!response.ok)throw new Error(response.status);const data=await response.json();if(data.articles?.length)articles=data.articles;const stamp=data.updatedAt?new Date(data.updatedAt).toLocaleString("zh-CN",{hour12:false}):"等待首次更新";}
